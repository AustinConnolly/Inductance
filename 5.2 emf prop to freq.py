...
sum_xy = np.sum(Freqdata*Vaxdata)
sum_x = np.sum(Freqdata)
sum_y = np.sum(Vaxdata)
sum_x2 = np.sum(Freqdata**2)
...
for i in range(N):
        d=Vaxdata[i]-(m*Freqdata[i]+c) # summation of d's
        sum_d2 += d**2
...
## Plotting ------------------------------------------------------------------------------------------------------------
plt.plot(Freqdata,Vaxdata,'.',label = 'Data points')
plt.title('')
plt.plot(x,y,'b',label='Best fit line')
xmin,xmax,ymin,ymax =  plt.axis ([0, 2010, 0, 0.5])
